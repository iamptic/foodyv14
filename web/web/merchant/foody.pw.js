/*! Foody PW unified (2025-08-18): .input--with-eye + .pwd-toggle (SVG icon, consistent) */
(function(){
  'use strict';
  ...
  function enhance(input){
    ...
    // Wrap input in .input--with-eye
    var wrap = input.closest('.input--with-eye');
    if (!wrap){
      wrap = document.createElement('div');
      wrap.className = 'input--with-eye';
      input.parentNode.insertBefore(wrap, input);
      wrap.appendChild(input);
    }
    // Add button .pwd-toggle if missing
    ...
    if (!btn){
      ...
      // ensure icon exists for consistency
      btn.innerHTML = EYE;
    }
  ...
  ready(function(){ scan(document); try{ new MutationObserver(function(m){ m.forEach(function(v){ v.addedNodes && v.addedNodes.forEach(function(n){ if(n.querySelectorAll) scan(n); }); }); }).observe(document.body,{childList:true,subtree:true}); }catch(_){}});
})();